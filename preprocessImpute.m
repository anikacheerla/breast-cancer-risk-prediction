...
table3 = dataread('file', 'imputedData.csv', '%s', 'delimiter', '\n'); % imputed table (imputation done in R)
    ...
    if (~remove_image(i))
        ...
        for j = 2: numel(table2)
            ...
            if (strcmp(table2_temp1(1), table1_temp2(1)))
                race(i) = str2double(table3_temp(3)); 
                age_cat(i) = str2double(table3_temp(4)); 
                mens_status(i) = str2double(table3_temp(8));
                birads(i) = str2double(table3_temp(7));
                volpara_dv(i) = str2double(table3_temp(27));
                volpara_pd(i) = str2double(table3_temp(28));
                bmi(i) = str2double(table3_temp(12));
                parity(i) = str2double(table3_temp(9));
